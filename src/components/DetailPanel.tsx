...
export default function DetailPanel({
  ...
  // Reset panel scroll each time we open a new day
  useEffect(() => {
  ...
  const handleClose = () => {
    setIsVisible(false);
    // Match the 300ms transition
    setTimeout(onClose, 300);
  };

  const handlePrevDay = () => {
    if (!weatherData?.dailyForecast || !selectedDay) return;
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          zIndex: 1010,
          // This helps on iPhones with notches (e.g., iPhone 14 Pro)
          paddingTop: 'env(safe-area-inset-top)',
          paddingBottom: 'env(safe-area-inset-bottom)',
        }}
      >
        {/* Header with close + day nav */}
        <div className="p-3 md:p-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={
                !weatherData?.dailyForecast?.some(
                  (day) => new Date(day.date) < new Date(selectedDay.date)
                )
              }
            >
              <svg
        ...
        {/* Scrollable content */}
        <div ref={panelRef} className="flex-1 overflow-y-auto px-3 md:px-6 pb-6">
          <Suspense fallback={<LoadingFallback />}>
            ...
            {/* Main summary */}
            <div className="p-2.5 md:p-3 rounded-lg md:rounded-xl backdrop-blur-md bg-white/5 mb-4 md:mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xl md:text-2xl font-bold">
                    {convertTemp(selectedDay.temp.min, tempUnit)}° -{' '}
                    {convertTemp(selectedDay.temp.max, tempUnit)}°{tempUnit}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6">
              <div className="p-2.5 md:p-3 bg-white/5 rounded-lg">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Image
                    src="/icons/weathers/raindrops.svg"
                    alt="Precipitation"
                    width={56}
                    height={56}
                    className="w-12 md:w-14 h-12 md:h-14 opacity-80"
              ...
              <div className="overflow-x-auto pb-4">
                <div className="inline-flex gap-3">
                  {hourlyData.map((hour, index) => (
                    <div
                      key={index}
                      className="p-2.5 md:p-3 bg-white/5 rounded-lg min-w-[120px] flex flex-col items-center gap-2 cursor-pointer transition-all duration-300 hover:bg-white/10 hover:shadow-lg relative group overflow-hidden"
                    >
