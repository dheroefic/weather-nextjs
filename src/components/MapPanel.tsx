...
export default function MapPanel({
  ...
  useEffect(() => {
    if (mapInstance && location.coordinates) {
      ...
      const handleMoveStart = () => {
        // Removed isDragging update since it's not used
      };
      ...
      const handleMoveEnd = () => {
        ...
        // Set a new timeout to fetch data after user stops moving the map
        debounceTimeoutRef.current = setTimeout(() => {
          const center = mapInstance.getCenter();
          fetchNearbyData(center.lat, center.lng);
        }, 1000); // Wait 1 second after movement stops before fetching
      };
  ...
  // Now, after all hooks are called, conditionally render based on coordinates.
            ...
            {shouldRenderMap && (
              <div className="h-full w-full leaflet-container-custom">
                <Suspense fallback={<LoadingFallback />}>
                  ...
                  >
                    {/* Set the map instance using a child component */}
                    ...
                    {nearbyLocations.map((loc: NearbyLocation) => (
                      <Marker
                        ...
                        icon={new L.Icon({
                          iconUrl: loc.weatherData?.currentWeather.icon || '/icons/weathers/map-marker.svg',
                          iconSize: [32, 32],
                          iconAnchor: [16, 16],
                          popupAnchor: [0, -16],
