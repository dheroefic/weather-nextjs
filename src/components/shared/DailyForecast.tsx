...
// Memoized forecast period buttons component
const ForecastButtons = memo(function ForecastButtons({
  forecastPeriod,
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-5 md:mb-8 gap-4 md:gap-0">
        ...
        <ForecastButtons forecastPeriod={forecastPeriod} onForecastPeriodChange={onForecastPeriodChange} />
      </div>
        ...
        <div className="inline-grid grid-flow-col auto-cols-[minmax(150px,1fr)] md:auto-cols-[minmax(220px,1fr)] gap-3 md:gap-5 pb-4">
          {loading ? loadingPlaceholders : (
            filteredForecast.map((day, index) => (
              <DayCard key={`${day.date}-${index}`} day={day} tempUnit={tempUnit} convertTemp={convertTemp} onDaySelect={onDaySelect} />
            ))
          )}
        </div>
      </div>
